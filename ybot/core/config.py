...
[commands]
enabled = true                             # 是否启用指令系统
admins = []                                # 管理员 QQ 号列表，如 [123456789, 987654321]
non_admin_behavior = "ignore"              # 非管理员发送指令的处理方式：
                                           #   "ignore"  — 忽略指令前缀，正常唤起模型参与对话
                                           #   "block"   — 拦截并完全无响应
require_at_in_group = true                 # 群聊中是否需要 @BOT 才能触发指令
                                           #   true  — 群聊必须 @BOT + 指令才触发
                                           #   false — 群聊中直接发送指令即可触发（无需 @BOT）
"""
...
class CommandsConfig:
    ...
    non_admin_behavior: str = "ignore"  # "ignore" | "block"
...
class Config:
    ...
    def _from_dict(cls, data: dict) -> Config:
        ...
        commands = CommandsConfig(
            enabled=commands_data.get("enabled", True),
            admins=commands_data.get("admins", []),
            non_admin_behavior=commands_data.get("non_admin_behavior", "ignore"),
            require_at_in_group=commands_data.get("require_at_in_group", True),
        )

        return cls(server=server, bot=bot, ai=ai, interceptor=interceptor, worldbook=worldbook, poke=poke, tools=tools, commands=commands)
